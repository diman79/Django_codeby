...
from .forms import CourseForm, LessonForm, OrderByAndSearchForm, SettingForm
...
def api_courses(request):
    ...
    return JsonResponse(data=dict(courses=json.loads(serialized_courses)), safe=False, json_dumps_params={'ensure_ascii': False, 'indent': 4})


def api_lessons(request):
    lessons = Lesson.objects.all()
    serialized_lessons = LessonSerializer().serialize(queryset=lessons,
                                                      use_natural_foreign_keys=True)
...
def api_trackings(request):
    ...
    serialized_trackings = TrackingSerializer().serialize(queryset=trackings,
                                                          use_natural_foreign_keys=True)
...
def api_reviews(request):
    ...
    serialized_reviews = ReviewSerializer().serialize(queryset=reviews,
                                                      use_natural_foreign_keys=True)
